...
class RoomCategoriesController extends Controller{
    ...
    public function getList(Request $request){
        ...
        return view('/panel/room_categories/index');
    }
    public function listPaginate(Request $request){
        if(!$request->session()->has('admin_email')){
            return redirect('/panel/');
        }
        $query = self::$RoomCategories->where('status', '!=', 3);
        ...
        return view('/panel/room_categories/paginate', compact('records'));
    }
        ...
        if($request->input()){
            ...
            if($validator->fails()){
				...
				if($request->input('row_id') == 0){
					$ordering = 1;
					$maxOrdering = self::$RoomCategories->max('ordering');
					if($maxOrdering > 0){
				...
				if($request->input('row_id') > 0){
					self::$RoomCategories->where('id',$request->row_id)->update($setData);
					echo json_encode(array('heading' => 'Success', 'msg' => 'Record updated successfully'));die;
				}else{
					$record = self::$RoomCategories->CreateRecord($setData);
					echo json_encode(array('heading' => 'Success', 'msg' => 'Record added successfully'));die;
				}
				
            }
        }
        return view('/panel/room_categories/add-page');
    }
    #getPage
    public function getPage(Request $request){
		...
		if($validator->fails()){
			...
			$record = self::$RoomCategories->where('id',$request->rowId)->first();
