...
class AmenityCategoriesController extends Controller{
    ...
    public function getList(Request $request){
        ...
        return view('/panel/amenity_categories/index');
    }
    public function listPaginate(Request $request){
        if(!$request->session()->has('admin_email')){
            return redirect('/panel/');
        }
        $query = self::$AmenityCategories->where('status', '!=', 3);
        ...
        return view('/panel/amenity_categories/paginate', compact('records'));
    }
        ...
        if($request->input()){
            ...
            if($validator->fails()){
				...
				if($request->input('row_id') > 0){
					self::$AmenityCategories->where('id',$request->row_id)->update($setData);
					echo json_encode(array('heading' => 'Success', 'msg' => 'Record updated successfully'));die;
				}else{
                    
                    $setData['status'] = 1;
					$record = self::$AmenityCategories->CreateRecord($setData);
					echo json_encode(array('heading' => 'Success', 'msg' => 'Record added successfully'));die;
				}
            }
        }
        return view('/panel/amenity_categories/add-page');
    }
    #getPage
    public function getPage(Request $request){
		...
		if($validator->fails()){
			...
			$record = self::$AmenityCategories->where('id',$request->rowId)->first();
