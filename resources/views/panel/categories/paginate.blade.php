@if($records->count()>0)
    ...
    @endphp
    <a style="cursor:pointer" onclick="changeStatus('categories','{!!$row->id!!}');" id="status_{{$row->id}}" class="badge {{$class}} me-1">{{$label}}</a>
    ...
    <a class="dropdown-item" onclick="deleteData('categories','{{ $row->id }}');" href="javascript:void(0);"><i class="icon-base ti tabler-trash me-1"></i> Delete</a>
    </div>
    </div>
    </td>
    </tr>
