@if($records->count()>0)
   ...
   @endphp
    <tr>
        <td>{{$sr }}.</td>
        <td>{{$row->title}}</td>
        <td>{{ucfirst($row->facility_type)}}</td>
        <td>
        @php
        if($row->status == 1){$class = 'bg-label-success'; $label = 'Active';}else{$class = 'bg-label-danger'; $label = 'In-Active';}
        @endphp
        <a style="cursor:pointer" onclick="changeStatus('premium_facilities','{!!$row->id!!}');" id="status_{{$row->id}}" class="badge {{$class}} me-1">{{$label}}</a>
        ...
        <div class="dropdown-menu">
            ...
            <a class="dropdown-item" onclick="deleteData('premium_facilities','{{ $row->id }}');" href="javascript:void(0);"><i class="icon-base ti tabler-trash me-1"></i> Delete</a>
            </div>
            </div>
        </td>
    </tr>
