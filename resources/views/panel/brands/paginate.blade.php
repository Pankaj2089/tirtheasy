...
@if($records->count()>0)
    ...
    <td>{{$row->title}}</td>
     <td>{{$row->type}}</td>
     <td>@if($row->banner != '')<img src="{{$siteUrl}}/public/admin/images/brands/{{$row->banner}}" width="100px" />@endif</td>
    
    <td>
    @php
    if($row->status == 1){$class = 'bg-label-success'; $label = 'Active';}else{$class = 'bg-label-danger'; $label = 'In-Active';}
    @endphp
    <a style="cursor:pointer" onclick="changeStatus('brands','{!!$row->id!!}');" id="status_{{$row->id}}" class="badge {{$class}} me-1">{{$label}}</a>
    ...
    <a class="dropdown-item" onclick="deleteData('brands','{{ $row->id }}');" href="javascript:void(0);"><i class="icon-base ti tabler-trash me-1"></i> Delete</a>
    </div>
    </div>
    </td>
    </tr>
