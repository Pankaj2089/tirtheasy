...
@section('content')
            ...
            <a style="color:#FFF" onclick="resetFilterForm();" class="btn btn-danger waves-effect waves-light">Reset</a>
              ...
              <div class="card">
                <h5 class="card-header">Distributers <a onclick="$('#popupHeading').html('Add Distributer');clearForm();" style="float:right; color:#FFF" data-bs-toggle="modal" data-bs-target="#addNewCCModal" class="btn btn-success waves-effect waves-light">Add</a></h5>
...
var addUrl = "{{route('admin.add-distributer')}}";
var getUrl = "{{route('admin.get-distributer')}}";
...
function getDetails(rowId){
	$('#popupHeading').html('Edit Distributer');
	$.ajax({
		headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
		type: 'POST',
		data:{rowId:rowId},
...
function filterData(type = null){
	...
	$.ajax({
		...
		url: "{{ url('/panel/distributers_paginate') }}",
		success: function(response){
