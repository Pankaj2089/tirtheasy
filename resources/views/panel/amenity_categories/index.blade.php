...
@section('content')
  ...
  <a style="color:#FFF" onclick="resetFilterForm();" class="btn btn-danger waves-effect waves-light">Reset</a>
    ...
    <div class="card">
      <h5 class="card-header">Amenity Categories <a onclick="$('#popupHeading').html('Add New Amenity Category');clearForm();" style="float:right; color:#FFF" data-bs-toggle="modal"
        ...
        <div class="modal-content">
          <div class="modal-body">
            ...
            <div class="text-center mb-6">
              <h4 id="popupHeading" class="mb-2">Add New Amenity Category</h4>
...
var addUrl = "{{route('admin.add-amenity-category')}}";
var getUrl = "{{route('admin.get-amenity-category')}}";
...
function getDetails(rowId){
	$('#popupHeading').html('Edit Amenity Category');
	$.ajax({
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        type: 'POST',
		data:{rowId:rowId},
...
function filterData(type = null){
	...
	$.ajax({
		...
		url: "{{ url('/panel/amenity_categories_paginate') }}",
		success: function(response){
