...
@section('content')
  ...
  <a style="color:#FFF" onclick="resetFilterForm();" class="btn btn-danger waves-effect waves-light">Reset</a>
    ...
    <div class="card">
      <h5 class="card-header">Facilities <a onclick="$('#popupHeading').html('Add New Facility');clearForm();" style="float:right; color:#FFF" data-bs-toggle="modal"
        ...
        <div class="modal-content">
          <div class="modal-body">
            ...
            <div class="text-center mb-6">
              <h4 id="popupHeading" class="mb-2">Add New Facility</h4>
              <!--<p>Create a brand new category from here.</p>-->
            </div>
            <form id="pageForm" class="row g-6" method="get">
...
var addUrl = "{{route('admin.add-facility')}}";
var getUrl = "{{route('admin.get-facility')}}";
...
function getDetails(rowId){
	$('#popupHeading').html('Edit Facility');
	$.ajax({
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        type: 'POST',
		data:{rowId:rowId},
...
function filterData(type = null){
	...
	$.ajax({
		...
		url: "{{ url('/panel/facilities_paginate') }}",
		success: function(response){
